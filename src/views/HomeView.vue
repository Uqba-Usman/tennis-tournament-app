...
const { tournaments, tournamentInProgress } = storeToRefs(tournamentStore);
...
<template>
  <div class="flex flex-col gap-5 px-4 pt-5">
    ...
    >
      ...
      <h2 class="mt-1 text-xl font-bold">{{ tournamentInProgress.name }}</h2>
      <p class="mt-1 text-sm text-white/85">
        {{ tournamentInProgress.playerIds.length }} players &middot; {{ tournamentInProgress.courtCount }} courts
      </p>
      <RouterLink
        :to="`/tournament/${tournamentInProgress.id}`"
